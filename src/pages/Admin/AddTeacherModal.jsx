...
const AddTeacherModal = ({ isOpen, onClose, onSuccess, teacherToEdit }) => {
  ...
  // ✅ 1. Auto-generate password on completely new modal load
  useEffect(() => {
    if (!isOpen) return;

    if (teacherToEdit) {
      setFormData({ ...INITIAL_DATA, ...teacherToEdit, password: '' });
      setEmailEditedManually(true); // Don't auto-override existing email
    } else {
      setFormData({ ...INITIAL_DATA, password: generatePassword() });
  ...
  // ✅ 2. Auto-generate email based on Full Name
  useEffect(() => {
    if (!emailEditedManually && formData.fullName && !teacherToEdit) {
      // Split, lowercase, remove special characters, and join with '.'
      const formattedName = formData.fullName.trim().toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, '.');
      if (formattedName) {
        setFormData(prev => ({ ...prev, email: `${formattedName}@radheshyam.edu` }));
      } else {
        setFormData(prev => ({ ...prev, email: '' }));
      }
    }
  }, [formData.fullName, emailEditedManually, teacherToEdit]);

  const handleManualPasswordGenerate = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'email') setEmailEditedManually(true);
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach(key => {
      // ✅ Explicitly append all valid string/number fields, including email/password
    ...
    try {
      ...
      // Keep track of the credentials used to pass back up
      const generatedEmail = formData.email;
      const generatedPassword = formData.password;
      const teacherName = formData.fullName;
      ...
      if (teacherToEdit) {
        ...
        // Return without credentials popup (password wasn't generated newly)
        onSuccess(null);
        onClose();
        ...
        // Pass the raw credentials up so the parent component can render the WhatsApp popup!
        onSuccess({
          name: teacherName,
                  ...
                  <div className="space-y-4 lg:pl-10 lg:border-l border-slate-100">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex gap-2 relative">
                        ...
                        <button
                          type="button"
                          onClick={handleManualPasswordGenerate}
